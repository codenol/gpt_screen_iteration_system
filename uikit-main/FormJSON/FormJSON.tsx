...
import { TExternalValidatorFunctions, useSchemaValidator } from './utils/useSchemaValidator';
...
export interface IFormJSON {
    ...
    /**
     * Колбек при ошибке загрузки валидатора. (работает только при проблемах с внешним валидатором)
     * @param error 
     * @returns 
     */
    onValidatorLoadError?: (error: Error | unknown) => void;

    /**
     * Объект с функциями для подключения внешнего валидатора.
     * 
     * @example
     * {
     *   getValidatorScriptSrc: (id) => `/schemas/api/v5/validators/${id}.js`,
     *   getValidatorId: async (schema) => $api({
     *       method: 'post',
     *       url: '/schemas/api/v5/validators',
     *       data: { schema },
     *   })
     * }
     */
    externalValidator?: TExternalValidatorFunctions
}
    ...
    (
        ...
        useEffect(() => {
            ...
            if (validationSchema?.length) {
                tempSteps.push(
                    ...validationSchema.map((item, index) => ({ label: item.label, disabled: progressSteps < index }))
                );
            }

            if (showStepValidation) {
                tempSteps.push({ label: 'Проверка', disabled: progressSteps < validationSchema?.length });
            }
            setSteps(tempSteps);
        }, [progressSteps]);
        ...
        const {
            ...
            isLoading,
            error,
        } = useSchemaValidator(formSchema, externalValidator);

        useEffect(() => {
            if (error && onValidatorLoadError)
                onValidatorLoadError(error)
        }, [error, onValidatorLoadError])

        if (isLoading && !validator) {
            return <div style={{ width: '100%', display: 'flex' }}>
                <ProgressSpinner strokeWidth="3" style={{ width: '80px', height: '80px', alignSelf: 'center' }} />
            </div>
        }
            ...
            <>
                ...
                {showStepValidation && step === steps.length - 1 ? (
                    <>
                        ...
                        >
                            {validationSchema?.map((item, index) => (
                    ...
                    <CustomForm
                        ...
                        schema={validationSchema?.length ? validationSchema[step] : validationSchema}
                        formData={formData}
                        ref={(node) => {
                            if (formRef) {
                        ...
                        uiSchema={validationSchema?.length ? validationSchema[step].uiSchema : validationSchema?.uiSchema}
                        showErrorList={false}
                        widgets={widgets}
                        fields={fields}
