...
export const UIkitProvider: React.FC<UIkitProviderProps> = (props) => {
    const { children, blockUIParams, withAbsoluteTooltip = false, absoluteTooltipProps } = props;
    usePrimeReactSettings();

    return (
        <>
            <ToastProvider>
                <BlockUIProvider {...blockUIParams}>
                    <ContextMenuProvider>
                        <ConfirmDialogProvider>{children}</ConfirmDialogProvider>
                    </ContextMenuProvider>
                </BlockUIProvider>
            </ToastProvider>
            {withAbsoluteTooltip && <AbsoluteTooltip {...absoluteTooltipProps} />}
        </>
    );
};
